        ...
        void DrawIndicators()
        {
            foreach (Target target in targets)
            {
                ...
                if (indicator)
                {
                    ...
                    if (target.showNameText)
                        indicator.SetDistanceTextWithName(distanceFromCamera, target._name);
                    else
                        indicator.SetDistanceText(distanceFromCamera); //Set the distance text for the indicator.
                    indicator.SetDistanceTextColor(target.TextColor);
                    indicator.transform.position = screenPosition; //Sets the position of the indicator on the screen.
                    indicator.SetTextRotation(Quaternion.identity); // Sets the rotation of the distance text of the indicator.
                    if (target.overRideIcon)
                    {
                        if (distanceFromCamera > 15000 && target.opacityWithDistance)
                        {
                            indicator.SetImage(target.distanceIcon);
                            indicator.SetDistanceTextColor(new Color(0, 0, 0, 0));
                            indicator.SetImageColor(target.TargetColor);
                        }
                            
                        else
                            indicator.SetImage(target.newIcon);
                    }
                    else
                        indicator.ResetToDefaultSprite();
                }
                if (target.opacityWithDistance && distanceFromCamera < 15000 && indicator != null)
                {
                     indicator.SetImageOpacity(distanceFromCamera); 
                }
            }
        }
